...
$next_pay    = $contrib['next_payment_date'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="auth-container" style="max-width:500px;">
  <div class="auth-logo">
    ...
    <p class="brand-tagline">Contribution Payment</p>
  </div>
  <div class="auth-card">
    <h2>Make Your Contribution</h2>
    <p class="auth-subtitle">
      Member ID: <strong style="color:var(--gold)"><?= htmlspecialchars($user['user_code'] ?? '') ?></strong>
    </p>
    <div class="payment-summary">
      ...
      <div class="payment-summary__row">
        ...
        <strong>Cycle <?= $cycle_num ?> of <?= $contrib['total_participants'] ?></strong>
      </div>
      <div class="payment-summary__row">
        <span>Your Collection Day</span>
      ...
      <?php if ($next_pay): ?>
      <div class="payment-summary__row">
        <span>This payment due</span>
        <strong><?= date('M j, Y', strtotime($next_pay)) ?></strong>
      </div>
      <?php endif; ?>
      <div class="payment-summary__row">
        <span>Reference</span>
        <strong style="font-family:monospace;font-size:.82rem"><?= $ref ?></strong>
      </div>
    </div>
    <button onclick="payWithPaystack()" class="btn btn-primary btn-full" id="pay-btn">
      Pay <?= formatMoney($contrib['contribution_amount']) ?>
    </button>
...
function payWithPaystack() {
  ...
  var handler = PaystackPop.setup({
    ...
    metadata: { custom_fields: [
      ...
      { display_name: 'Cycle',     variable_name: 'cycle',     value: '<?= $cycle_num ?>' }
    ]},
    callback: function(res) {
      window.location.href = '<?= SITE_URL ?>/payments/paystack_verify.php?cid=<?= $cid ?>&reference=' + res.reference;
    },
    onClose: function() {
      document.getElementById('pay-btn').disabled = false;
      document.getElementById('pay-btn').textContent = 'Pay <?= formatMoney($contrib['contribution_amount']) ?>';
    }
  });
  handler.openIframe();
}
